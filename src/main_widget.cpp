...
void Main_Widget::init()
{
    ...
    cfgFrame = new QFrame();
    cfgFrame->setGeometry ( 50, 50, 380, 600 );
    cfgFrame->setMinimumWidth ( 380 );
    cfgFrame->setMaximumWidth ( 380 );
    cfgFrame->setMinimumHeight ( 600 );
    cfgFrame->setMaximumHeight ( 600 );
    ...
    cfgSpectrogram->setGeometry( 5, 360, 330, 140 );
    ...
    cfgSpecScrollingLabel->setGeometry ( 10, 40, 160, 20 );
    ...
    cfgSpecScrolling->setGeometry ( 180, 40, 150, 20 );
    ...
    cfgspectrogramAvgAttackLabel->setGeometry ( 10, 80, 160, 20 );
    ...
    SpectrogramAvgAttackSpinBox->setGeometry ( 180, 80, 70, 20 );
    ...
    cfgspectrogramAvgDecayLabel->setGeometry ( 10, 100, 160, 20 );
    ...
    SpectrogramAvgDecaySpinBox->setGeometry ( 180, 100, 70, 20 );
...
#if 0
    QLabel *cfgspectrogramNumAVGLabel = new QLabel ( cfgSpectrogram );
    cfgspectrogramNumAVGLabel->setText ( "Averaging Samples: " );
    cfgspectrogramNumAVGLabel->setGeometry ( 10, 100, 160, 20 );
    cfgspectrogramNumAVGLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    SpectrogramNumAVGInput = new QSpinBox ( cfgSpectrogram );
    SpectrogramNumAVGInput->setGeometry ( 180, 100, 70, 20 );
    SpectrogramNumAVGInput->setMinimum ( 1 );
    SpectrogramNumAVGInput->setMaximum ( 400 );
    SpectrogramNumAVGInput->setValue ( spectrogramNumAVG );
    connect( SpectrogramNumAVGInput, SIGNAL( valueChanged ( int ) ),
             this, SLOT ( updateSpectrogramNumAVG ( int ) ) );
#endif

    // no3m
    QGroupBox *cfgColorPalette = new QGroupBox( cfgFrame3 );
    cfgColorPalette->setTitle("Color Palette");
    cfgColorPalette->setGeometry( 5, 520, 330, 40 );
...
void Main_Widget::plotSpectrum( int y )
{
    ...
    char f_text[10];
    bool debug = false;
    //int spectrum_avg[def_spec];
    static int spectrum_avg[MAX_DEFSPEC];
    int spectrum_smoothing[def_spec];
    int spectrum_display[spectrumFrame->width()];
    memset(spectrum_display, 0, sizeof(int) * spectrumFrame->width());
...
// xxxx
#if 1
    if (first) {
       memset(spectrum_avg, 0, sizeof(int) * MAX_DEFSPEC);
       first = false;
    }
    // exponential moving average (IIR filter) in time-domain of spectrum data // no3m
    if (specAveraging > 0) {
       float a = 2.0 / ( 1.0 + specAveraging );
       //float a;
       float attack = 0.9f;
       float decay = 0.3f;
       for ( x = 0; x < def_spec; x++ ) {
             //avg = ( current * a ) + (1 - a) * avg // exponential / IIR
             //if ( spectrum_avg[x] < spectrum_history[y][x] )
             //   a = attack;
             //else
             //   a = decay;
             spectrum_avg[x] = ( (spectrum_history[y][x] - spectrum_avg[x]) * a ) + spectrum_avg[x]; // exponential / IIR
             //spectrum_avg[x] = (((specAveraging - 1) * spectrum_avg[x]) + spectrum_history[y][x] ) / specAveraging; // simple
       }
    }
#endif
#if 0
  // Average the last N samples of spectrum data
    if (specAveraging > 1) {
        memset(spectrum_avg, 0, sizeof(int) * MAX_DEFSPEC);
        for (y1 = y, n=0; n < specAveraging && n < spectrogram->height(); n++) {
            for ( x = 1; x < def_spec; x++ ) {
                spectrum_avg[x] += spectrum_history[y1][x];
            }
            y1--;
            if (y1 < 0)
        ...
        for ( x = 1; x < def_spec; x++ ) {
            ...
            spectrum_avg[x] = spectrum_avg[x] / n;
        }
    }

    //y1 =  spectrogram->height() + spectrumFrame->height();
#endif
    //spectrum averages
    int spec_avg = 0;
    int spec_avg0 = 0;
    ...
    if (specAvgLine || specPeakMarkers || autoSpecAperture > 0) {
            ...
            // average a lot of samples to smooth out jitter
            int k = 0;
            for (y1 = y, n=0; n < 200 && n < SPECTRUM_HISTORY_SIZE; ++n) {
                for ( x = x1; x < x2; ++x ) {
                    spec_avg += spectrum_history[y1][x];
                }
                spec_avg /= (x2 - x1);
                if (n == 0) spec_avg0 = spec_avg;
                for ( x = x1; x < x2; ++x ) {
                   // remove signals
                   if (spectrum_history[y1][x] < spec_avg + 4) {
                      background_avg += spectrum_history[y1][x];
                      k++;
                   }
                }
                spec_avg = 0;
                y1--;
                if (y1 < 0)
                    y1 = SPECTRUM_HISTORY_SIZE - 1;
            }
            if (k > 0){
               background_avg /= k;
            } else {
               background_avg = spec_avg0;
            }
#endif
#if 1
        // original routine
        ...
        for (int i=x1; i < x2; ++i){
            ...
            if (specAveraging > 1)
                spec_tmp = spectrum_avg[i];
            else
                spec_tmp = spectrum_history[y][i];
...
// xxxx
    ...
    if (spectrumMode == SPEC_AVG || specPeakMarkers) {
#if 1
       // Gaussian smoothing across frequency domain
       memset(spectrum_smoothing, 0, sizeof(int) * def_spec);
       int bins = 3;
       int len = ( bins * 2 ) + 1;
       float coeff[ len ];
       gaussian_kernel ( coeff, bins );
       for ( int i = x1 ; i < x2 ; ++i ) {
          if ( i - bins > x1 && i + bins < x2 ) {
             for ( int j=0 ; j < len ; ++j ) {
                if (specAveraging > 1) {
                   spectrum_smoothing[ i ] += spectrum_avg[ i + j - bins ] * coeff[ j ];
                } else {
                   spectrum_smoothing[ i ] += spectrum_history[ y ][ i + j - bins ] * coeff[ j ];
                }
             }
          } else {
             if (specAveraging > 1) {
                spectrum_smoothing[ i ] = spectrum_avg[ i ];
             } else {
                spectrum_smoothing[ i ] = spectrum_history[ y ][ i ];
             }
          }
       }
#endif
#if 0
        // spectra smoothing across 5 bins
        memset(spectrum_smoothing, 0, sizeof(int) * def_spec);
        int k;
        for (int i=x1; i<x2; ++i) {
           k=0;
           for (int j = -2; j < 3; ++j) {
              if (i + j > 0 && i + j < def_spec) {
                 if (specAveraging > 1) {
                    //spectrum_smoothing[i] += spectrum_avg[i+j] * spectrum_avg[i+j]; // rms
                    spectrum_smoothing[i] += spectrum_avg[i+j]; // mean
                 } else {
                    //spectrum_smoothing[i] += spectrum_history[y][i+j] * spectrum_history[y][i+j]; // rms
                    spectrum_smoothing[i] += spectrum_history[y][i+j]; // mean
                 }
                 ++k;
              }
           }
           //spectrum_smoothing[i] = sqrt (spectrum_smoothing[i] / k); // rms
           spectrum_smoothing[i] /= k; // mean
        }
#endif
#if 0
        if (specAveraging > 1) {
            spectrum_smoothing[0] = (spectrum_avg[0] + spectrum_avg[1]) / 2;
            spectrum_smoothing[1] = (spectrum_avg[0] + spectrum_avg[1] + spectrum_avg[2]) / 3;
            spectrum_smoothing[def_spec-1] = (spectrum_avg[def_spec-1] + spectrum_avg[def_spec-2]) / 2;
            spectrum_smoothing[def_spec-2] = (spectrum_avg[def_spec-1] + spectrum_avg[def_spec-2]+ spectrum_avg[def_spec-3]) / 3;
            for (int i=2; i < def_spec-2; i++){
                spectrum_smoothing[i] = (spectrum_avg[i] + spectrum_avg[i+1] + spectrum_avg[i-1] +
                        spectrum_avg[i+2] + spectrum_avg[i-2]) / 5;
            }
        } else {
            spectrum_smoothing[0] = (spectrum_history[y][0] + spectrum_history[y][1]) / 2;
            spectrum_smoothing[1] = (spectrum_history[y][0] + spectrum_history[y][1] + spectrum_history[y][2]) / 3;
            spectrum_smoothing[def_spec-1] = (spectrum_history[y][def_spec-1] + spectrum_history[y][def_spec-2]) / 2;
            spectrum_smoothing[def_spec-2] = (spectrum_history[y][def_spec-1] + spectrum_history[y][def_spec-2] + spectrum_history[y][def_spec-3]) / 3;
            for (int i=2; i < def_spec-2; i++){
                spectrum_smoothing[i] = (spectrum_history[y][i] + spectrum_history[y][i+1] + spectrum_history[y][i-1] +
    ...
    if (spectrumMode == SPEC_PEAK) {
        ...
        for (int i=x1; i<x2; i++) {
            if (specAveraging > 1) {
                if (spectrum_avg[i] > spectrum_peak[i]) {
                    spectrum_peak[i] = spectrum_avg[i];
                }
            } else {
    ...
    for ( x = 0 ; x < spectrum_width + 1 ; ) {
        ...
        if (hScale < 0.99) {
           if (spectrumMode == SPEC_PEAK)
              ...
              slope = float ( spectrum_smoothing[ x_next + x1 ] - spectrum_smoothing[ x + x1 ] ) / float ( sx_next - sx );
        ...
        if (hScale > 0.99) { // 1:1 or zoomed out
           ...
           for (int i = 0; i < x_next - x ; ++i) {
              if (spectrumMode == SPEC_PEAK && spectrum_peak[ x + x1 + i ] > max)
                    max = spectrum_peak[ x + x1 + i ];
              else if (spectrumMode == SPEC_AVG && spectrum_smoothing[ x + x1 + i ] > max)
                    max = spectrum_smoothing[ x + x1 + i ];
              else if (specAveraging > 1 && spectrum_avg[ x + x1 + i ] > max)
                    max = spectrum_avg[ x + x1 + i ];
              else if (spectrum_history[y][ x + x1 + i ] > max)
        ...
        if (hScale < 0.99) { // zoomed in; interpolate missing bins with simple linear slope
            for (int i = 0; i < sx_next - sx && sx + i < spectrumFrame->width() ; ++i) {
               if (spectrumMode == SPEC_PEAK)
                  ...
                  spectrum_display[sx+i] = int ( float (spectrum_smoothing[ x + x1 ]) + ( slope * float (i) ) );
               else if (specAveraging > 1)
                  spectrum_display[sx+i] = int ( float (spectrum_avg[ x + x1 ]) + ( slope * float (i) ) );
               else
                  spectrum_display[sx+i] = int ( float (spectrum_history[y][ x + x1 ]) + ( slope * float (i) ) );
            }
        }
