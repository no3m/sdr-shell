...
void Main_Widget::init()
{
    ...
    spectrogram = new Spectrum ( spectrogramFrame );
    connect ( spectrogram, SIGNAL ( zoom ( int ) ),
              this, SLOT ( zoom_mouse ( int ) ) );
    connect ( spectrogram, SIGNAL ( movement ( int ) ),
              this, SLOT ( f_at_mousepointer ( int ) ) );
    connect ( spectrogram, SIGNAL ( resetZoom ( int ) ),
              this, SLOT ( resetZoom ( int ) ) );
    connect ( spectrogram, SIGNAL ( tune ( int ) ),
              this, SLOT ( spectrogramClicked ( int ) ) );
    connect ( spectrogram, SIGNAL ( scale ( int ) ),
              this, SLOT ( changeFreqScale ( int ) ) );
    //connect ( spectrogram, SIGNAL ( pan ( int ) ),
    ...
    // -----------------------------------------------------------------------
    // Frequency Scale
    freqScale = new Spectrum ( spectrogramFrame );
    connect ( freqScale, SIGNAL ( zoom ( int ) ),
              this, SLOT ( zoom_mouse ( int ) ) );
    connect ( freqScale, SIGNAL ( movement ( int ) ),
              this, SLOT ( f_at_mousepointer ( int ) ) );
    connect ( freqScale, SIGNAL ( resetZoom ( int ) ),
              this, SLOT ( resetZoom ( int ) ) );
    connect ( freqScale, SIGNAL ( tune ( int ) ),
              this, SLOT ( spectrogramClicked ( int ) ) );
    connect ( freqScale, SIGNAL ( scale ( int ) ),
              this, SLOT ( changeFreqScale ( int ) ) );
    connect ( freqScale, SIGNAL ( pan ( int ) ),
              this, SLOT ( setSpecOffset ( int ) ) );
    ...
    connect ( spectrumFrame, SIGNAL ( zoom ( int ) ),
              this, SLOT ( zoom_mouse ( int ) ) );
    connect ( spectrumFrame, SIGNAL ( movement ( int ) ),
              this, SLOT ( f_at_mousepointer ( int ) ) );
    connect ( spectrumFrame, SIGNAL ( resetZoom ( int ) ),
              this, SLOT ( resetZoom ( int ) ) );
    connect ( spectrumFrame, SIGNAL ( tune ( int ) ),
              this, SLOT ( spectrogramClicked ( int ) ) );
    connect ( spectrumFrame, SIGNAL ( scale ( int ) ),
              this, SLOT ( changeFreqScale ( int ) ) );
    connect ( spectrumFrame, SIGNAL ( pan ( int ) ),
              this, SLOT ( setSpecOffset ( int ) ) );
    ...
    ctlFrame->setFrameStyle ( QFrame::NoFrame  | QFrame::Plain );
    ...
    cfgFrame = new QFrame();
    cfgFrame->setGeometry ( 50, 50, 360, 550 );
    cfgFrame->setMinimumWidth ( 360 );
    cfgFrame->setMaximumWidth ( 360 );
    cfgFrame->setMinimumHeight ( 550 );
    cfgFrame->setMaximumHeight ( 550 );
    cfgFrame->setWindowTitle("SDR-Shell : Config ");
    cfgFrame->move(geometry.x() + geometry.width()/2 - cfgFrame->width()/2, geometry.y() + geometry.height()/2 - cfgFrame->height()/2);
    ...
    QFrame *cfgFrame8 = new QFrame( cfgFrame );
    QFrame *cfgFrame9 = new QFrame( cfgFrame );
    ...
    QTabWidget *spectrumTab = new QTabWidget ( cfgFrame3 );
    spectrumTab->setFont ( *font1 );
    spectrumTab->addTab ( cfgFrame8, "Spectrum" );
    spectrumTab->addTab ( cfgFrame9, "Spectrogram" );
    spectrumTab->setGeometry ( 2, 150, 348, 355 );
    ...
    captureDirectoryLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    captureDirectoryInput = new QLineEdit ( cfgCaptureBox );
    captureDirectoryInput->setGeometry( 90, 18, 120, 20 );
    captureDirectoryInput->setText ( capture_directory );
    QPushButton *updateCaptureDirectoryButton = new QPushButton ( cfgCaptureBox );
    updateCaptureDirectoryButton->setText ( "Update" );
    updateCaptureDirectoryButton->setGeometry ( 220, 18, 70, 20 );
    updateCaptureDirectoryButton->setEnabled(true);
    connect ( updateCaptureDirectoryButton, SIGNAL ( pressed() ),
              this, SLOT ( updateCaptureDirectory() ) );
    cfgCaptureAuto = new QRadioButton ( cfgCaptureBox );
    cfgCaptureAuto->setText ( "Auto Capture" );
    cfgCaptureAuto->setGeometry ( 10, 40, 200, 20 );
    cfgCaptureAuto->setAutoExclusive(false);
    connect ( cfgCaptureAuto, SIGNAL ( toggled ( bool ) ),
    ...
    QLabel *cfgHwGainLabel = new QLabel ( cfgHwCal );
    cfgHwGainLabel->setText ( "Gain(dB): " );
    cfgHwGainLabel->setGeometry ( 10, 18, 70, 20 );
    cfgHwGainLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );

    cfgHwGain = new QDoubleSpinBox ( cfgHwCal );
    cfgHwGain->setGeometry ( 80, 18, 70, 20 );
    cfgHwGain->setMinimum( -20 );
    cfgHwGain->setMaximum( 20 );
    cfgHwGain->setSingleStep( 0.1 );
    cfgHwGain->setValue ( hwGain );
    ...
    QLabel *rxIqGainCourseLabel = new QLabel ( cfgIqGainSliders);
    rxIqGainCourseLabel->setText ( "Course: ");
    rxIqGainCourseLabel->setGeometry ( 10, 30, 70, 20 );
    rxIqGainCourseLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );

    rxIqGainCourseSlider = new QSlider (Qt::Horizontal, cfgIqGainSliders);
    rxIqGainCourseSlider->setGeometry ( 80, 30, 240, 20 );
    rxIqGainCourseSlider->setMinimum( -500 );
    rxIqGainCourseSlider->setMaximum( 500 );
    rxIqGainCourseSlider->setSingleStep( 10 );
    rxIqGainCourseSlider->setTickPosition(QSlider::NoTicks );
    rxIqGainCourseSlider->setValue ( iqGain );
    connect ( rxIqGainCourseSlider, SIGNAL ( valueChanged ( int ) ),
              this, SLOT ( updateIQGainSlider ( int ) ) );

    QLabel *rxIqGainFineLabel = new QLabel ( cfgIqGainSliders);
    rxIqGainFineLabel->setText ( "Fine: ");
    rxIqGainFineLabel->setGeometry ( 10, 60, 70, 20 );
    rxIqGainFineLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );

    rxIqGainFineSlider = new QSlider (Qt::Horizontal, cfgIqGainSliders);
    rxIqGainFineSlider->setGeometry ( 80, 60, 240, 20 );
    rxIqGainFineSlider->setMinimum( -10 );
    rxIqGainFineSlider->setMaximum( 10 );
    rxIqGainFineSlider->setSingleStep( 1 );
    rxIqGainFineSlider->setTickPosition(QSlider::NoTicks );
    rxIqGainFineSlider->setValue ( 0 );
    connect ( rxIqGainFineSlider, SIGNAL ( valueChanged ( int ) ),
              this, SLOT ( updateIQGainSlider ( int ) ) );

    QGroupBox *cfgIqPhaseSliders = new QGroupBox ( cfgFrame2 );
    cfgIqPhaseSliders->setTitle ( "RX IQ Phase Sliders (alt.)" );
    cfgIqPhaseSliders->setGeometry( 5, 250, 330, 100);

    QLabel *rxIqPhaseCourseLabel = new QLabel ( cfgIqPhaseSliders);
    rxIqPhaseCourseLabel->setText ( "Course: ");
    rxIqPhaseCourseLabel->setGeometry ( 10, 30, 70, 20 );
    rxIqPhaseCourseLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );

    rxIqPhaseCourseSlider = new QSlider (Qt::Horizontal, cfgIqPhaseSliders);
    rxIqPhaseCourseSlider->setGeometry ( 80, 30, 240, 20 );
    rxIqPhaseCourseSlider->setMinimum( -400 );
    rxIqPhaseCourseSlider->setMaximum( 400 );
    rxIqPhaseCourseSlider->setSingleStep( 10 );
    rxIqPhaseCourseSlider->setTickPosition(QSlider::NoTicks );
    rxIqPhaseCourseSlider->setValue ( iqPhase );
    connect ( rxIqPhaseCourseSlider, SIGNAL ( valueChanged ( int ) ),
              this, SLOT ( updateIQPhaseSlider ( int ) ) );

    QLabel *rxIqPhaseFineLabel = new QLabel ( cfgIqPhaseSliders);
    rxIqPhaseFineLabel->setText ( "Fine: ");
    rxIqPhaseFineLabel->setGeometry ( 10, 60, 70, 20 );
    rxIqPhaseFineLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );

    rxIqPhaseFineSlider = new QSlider (Qt::Horizontal, cfgIqPhaseSliders);
    rxIqPhaseFineSlider->setGeometry ( 80, 60, 240, 20 );
    rxIqPhaseFineSlider->setMinimum( -10 );
    rxIqPhaseFineSlider->setMaximum( 10 );
    rxIqPhaseFineSlider->setSingleStep( 1 );
    rxIqPhaseFineSlider->setTickPosition(QSlider::NoTicks );
    ...
    cfgFFTWindowGroup->setGeometry( 5, 101, 330, 45 );

    fftWindowBox= new QComboBox (cfgFFTWindowGroup);
    fftWindowBox->insertItem(0, "Rectangular");
    fftWindowBox->insertItem(1, "Hanning" );
    fftWindowBox->insertItem(2, "Welch" );
    fftWindowBox->insertItem(3, "Parzen" );
    fftWindowBox->insertItem(4, "Bartlett" );
    fftWindowBox->insertItem(5, "Hamming" );
    fftWindowBox->insertItem(6, "Blackman 2" );
    fftWindowBox->insertItem(7, "Blackman 3" );
    fftWindowBox->insertItem(8, "Blackman 4" );
    fftWindowBox->insertItem(9, "Exponential" );
    fftWindowBox->insertItem(10, "Riemann" );
    fftWindowBox->insertItem(11, "Blackman-Harris" );
    fftWindowBox->insertItem(12, "Nuttall" );
    fftWindowBox->setGeometry ( 10, 18, 140, 19 );
    fftWindowBox->setCurrentIndex(fftWindow);
    connect( fftWindowBox, SIGNAL( activated ( int ) ), this, SLOT ( setFFTWindow ( int ) ) );

    // Spectrum display
    QGroupBox *cfgSpecDisplay = new QGroupBox( cfgFrame8 );
    cfgSpecDisplay->setTitle("Spectrum Display");
    //cfgSpecDisplay->setGeometry( 5, 150, 370, 260 );
    cfgSpecDisplay->setGeometry( 5, 5, 370, 300 );
    ...
    spectrumOnTopButton->setGeometry ( 10, 100, 200, 20 );
    ...
    cfgSpecTimerLabel->setText ( "FFTs/sec: " );
    cfgSpecTimerLabel->setGeometry ( 10, 140, 160, 20 );
    cfgSpecTimerLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    cfgSpecTimerInput = new QSpinBox ( cfgSpecDisplay );
    cfgSpecTimerInput->setGeometry ( 180, 140, 70, 20 );
    ...
    cfgSpecScaleLabel->setText ( "Spectrum Scale (dB): " );
    cfgSpecScaleLabel->setGeometry ( 10, 160, 160, 20 );
    ...
    QLabel *cfgspectrumAvgAttackLabel = new QLabel ( cfgSpecDisplay );
    cfgspectrumAvgAttackLabel->setText ( "Avg (IIR) Attack: " );
    cfgspectrumAvgAttackLabel->setGeometry ( 10, 220, 160, 20 );
    cfgspectrumAvgAttackLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    SpectrumAvgAttackSpinBox = new QDoubleSpinBox ( cfgSpecDisplay );
    SpectrumAvgAttackSpinBox->setGeometry ( 180, 220, 70, 20 );
    SpectrumAvgAttackSpinBox->setMinimum( 0.01 );
    SpectrumAvgAttackSpinBox->setMaximum( 1.0 );
    SpectrumAvgAttackSpinBox->setDecimals( 2 );
    SpectrumAvgAttackSpinBox->setSingleStep( 0.01);
    SpectrumAvgAttackSpinBox->setValue ( ( double ) spectrumAvgAttack );
    connect ( SpectrumAvgAttackSpinBox, SIGNAL ( valueChanged (double ) ),
              this, SLOT ( updateSpectrumAvgAttack (double ) ) );
    QLabel *tmplabel1 = new QLabel (cfgSpecDisplay);
    tmplabel1->setText ( "(1=no avg)" );
    tmplabel1->setGeometry ( 255, 220, 80, 20 );
    tmplabel1->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );

    QLabel *cfgspectrumAvgDecayLabel = new QLabel ( cfgSpecDisplay );
    cfgspectrumAvgDecayLabel->setText ( "Avg (IIR) Decay : " );
    cfgspectrumAvgDecayLabel->setGeometry ( 10, 240, 160, 20 );
    cfgspectrumAvgDecayLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    SpectrumAvgDecaySpinBox = new QDoubleSpinBox ( cfgSpecDisplay );
    SpectrumAvgDecaySpinBox->setGeometry ( 180, 240, 70, 20 );
    SpectrumAvgDecaySpinBox->setMinimum( 0.01 );
    SpectrumAvgDecaySpinBox->setMaximum( 1.0 );
    SpectrumAvgDecaySpinBox->setDecimals( 2 );
    SpectrumAvgDecaySpinBox->setSingleStep( 0.01);
    SpectrumAvgDecaySpinBox->setValue ( ( double ) spectrumAvgDecay );
    connect ( SpectrumAvgDecaySpinBox, SIGNAL ( valueChanged (double ) ),
              this, SLOT ( updateSpectrumAvgDecay (double ) ) );
    QLabel *tmplabel2 = new QLabel (cfgSpecDisplay);
    tmplabel2->setText ( "(1=no avg)" );
    tmplabel2->setGeometry ( 255, 240, 80, 20 );
    tmplabel2->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );

    QLabel *cfgspectrumAvgGaussLabel = new QLabel ( cfgSpecDisplay );
    cfgspectrumAvgGaussLabel->setText ( "Gaussian Avg Bins: " );
    cfgspectrumAvgGaussLabel->setGeometry ( 10, 260, 160, 20 );
    cfgspectrumAvgGaussLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    SpectrumAvgGaussSpinBox = new QSpinBox ( cfgSpecDisplay );
    SpectrumAvgGaussSpinBox->setGeometry ( 180, 260, 70, 20 );
    SpectrumAvgGaussSpinBox->setMinimum( 0 );
    SpectrumAvgGaussSpinBox->setMaximum( 20 );
    SpectrumAvgGaussSpinBox->setSingleStep( 1 );
    SpectrumAvgGaussSpinBox->setValue ( ( int ) spectrumAvgGaussBins );
    connect ( SpectrumAvgGaussSpinBox, SIGNAL ( valueChanged ( int ) ),
              this, SLOT ( updateSpectrumAvgGaussBins ( int ) ) );
    QLabel *tmplabel3 = new QLabel (cfgSpecDisplay);
    tmplabel3->setText ( "(0=no avg)" );
    tmplabel3->setGeometry ( 255, 260, 80, 20 );
    tmplabel3->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );

    // Spectrogram display
    QGroupBox *cfgSpectrogram = new QGroupBox( cfgFrame9 );
    cfgSpectrogram->setTitle("Spectrogram Display");
    //cfgSpectrogram->setGeometry( 5, 440, 330, 160 );
    cfgSpectrogram->setGeometry( 5, 5, 330, 160 );
    ...
    cfgSpectrogramRefreshLabel->setText ( "FFTs/line: " );
    ...
    cfgspectrogramAvgAttackLabel->setGeometry ( 10, 100, 160, 20 );
    cfgspectrogramAvgAttackLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    SpectrogramAvgAttackSpinBox = new QDoubleSpinBox ( cfgSpectrogram );
    SpectrogramAvgAttackSpinBox->setGeometry ( 180, 100, 70, 20 );
    SpectrogramAvgAttackSpinBox->setMinimum( 0.01 );
    SpectrogramAvgAttackSpinBox->setMaximum( 1.0 );
    SpectrogramAvgAttackSpinBox->setDecimals( 2 );
    SpectrogramAvgAttackSpinBox->setSingleStep( 0.01);
    SpectrogramAvgAttackSpinBox->setValue ( ( double ) spectrogramAvgAttack );
    connect ( SpectrogramAvgAttackSpinBox, SIGNAL ( valueChanged (double ) ),
              this, SLOT ( updateSpectrogramAvgAttack (double ) ) );
    QLabel *tmplabel4 = new QLabel (cfgSpectrogram);
    tmplabel4->setText ( "(1=no avg)" );
    tmplabel4->setGeometry ( 255, 100, 80, 20 );
    tmplabel4->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );

    QLabel *cfgspectrogramAvgDecayLabel = new QLabel ( cfgSpectrogram );
    cfgspectrogramAvgDecayLabel->setText ( "Avg (IIR) Decay : " );
    cfgspectrogramAvgDecayLabel->setGeometry ( 10, 120, 160, 20 );
    cfgspectrogramAvgDecayLabel->setAlignment ( Qt::AlignRight | Qt::AlignVCenter );
    SpectrogramAvgDecaySpinBox = new QDoubleSpinBox ( cfgSpectrogram );
    SpectrogramAvgDecaySpinBox->setGeometry ( 180, 120, 70, 20 );
    SpectrogramAvgDecaySpinBox->setMinimum( 0.01 );
    SpectrogramAvgDecaySpinBox->setMaximum( 1.0 );
    SpectrogramAvgDecaySpinBox->setDecimals( 2 );
    SpectrogramAvgDecaySpinBox->setSingleStep( 0.01);
    SpectrogramAvgDecaySpinBox->setValue ( ( double ) spectrogramAvgDecay );
    connect ( SpectrogramAvgDecaySpinBox, SIGNAL ( valueChanged (double ) ),
              this, SLOT ( updateSpectrogramAvgDecay (double ) ) );
    QLabel *tmplabel5 = new QLabel (cfgSpectrogram);
    tmplabel5->setText ( "(1=no avg)" );
    tmplabel5->setGeometry ( 255, 120, 80, 20 );
    tmplabel5->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );

    QLabel *cfgspectrogramAvgGaussLabel = new QLabel ( cfgSpectrogram );
    cfgspectrogramAvgGaussLabel->setText ( "Gaussian Avg Bins: " );
    cfgspectrogramAvgGaussLabel->setGeometry ( 10, 140, 160, 20 );
    ...
    SpectrogramAvgGaussSpinBox->setGeometry ( 180, 140, 70, 20 );
    SpectrogramAvgGaussSpinBox->setMinimum( 0 );
    SpectrogramAvgGaussSpinBox->setMaximum( 20 );
    SpectrogramAvgGaussSpinBox->setSingleStep( 1 );
    SpectrogramAvgGaussSpinBox->setValue ( ( int ) spectrogramAvgGaussBins );
    connect ( SpectrogramAvgGaussSpinBox, SIGNAL ( valueChanged ( int ) ),
              this, SLOT ( updateSpectrogramAvgGaussBins ( int ) ) );
    QLabel *tmplabel6 = new QLabel (cfgSpectrogram);
    tmplabel6->setText ( "(0=no avg)" );
    tmplabel6->setGeometry ( 255, 140, 80, 20 );
    tmplabel6->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );

    // no3m
    QGroupBox *cfgColorPalette = new QGroupBox( cfgFrame9 );
    cfgColorPalette->setTitle("Color Palette");
    //cfgColorPalette->setGeometry( 5, 620, 330, 40 );
    cfgColorPalette->setGeometry( 5, 200, 330, 40 );
    cfgPalette = new QComboBox (cfgColorPalette);
    cfgPalette->insertItem(0, "SDR-shell");
    cfgPalette->insertItem(1, "GQRX" );
    cfgPalette->insertItem(2, "Linrad" );
    cfgPalette->insertItem(3, "Winrad" );
    cfgPalette->insertItem(4, "GHPSDR" );
    cfgPalette->insertItem(5, "Spectran" );
    cfgPalette->insertItem(6, "Spectrogram" );
    cfgPalette->insertItem(7, "SpectrumLab" );
    cfgPalette->insertItem(8, "WSJTX" );
    cfgPalette->insertItem(9, "Fldigi" );
    cfgPalette->insertItem(10, "Digipan" );
    cfgPalette->insertItem(11, "Gmfsk" );
    cfgPalette->insertItem(12, "Rocky" );
    cfgPalette->insertItem(13, "Horne" );
    cfgPalette->insertItem(14, "Light" );
    cfgPalette->insertItem(15, "Dark" );
    cfgPalette->insertItem(16, "SpecExt" );
    cfgPalette->insertItem(17, "Negative" );
    cfgPalette->insertItem(18, "Stereo 3D" );
    cfgPalette->insertItem(19, "NO3M Grey" );
    cfgPalette->insertItem(20, "NO3M GreyLin" );
    ...
    dspFrame1 = new QFrame( dspFrame );
    dspFrame2 = new QFrame( dspFrame );
    dspFrame3 = new QFrame( dspFrame );
    dspFrame4 = new QFrame( dspFrame );

    dspTab = new QTabWidget ( dspFrame );
    dspTab->addTab ( dspFrame1, "NR" );
    dspTab->addTab ( dspFrame2, "ANF" );
    dspTab->addTab ( dspFrame3, "NB" );
    dspTab->addTab ( dspFrame4, "Gain" );
    ...
    NR_GainLabel->setText ( "Adaptation Rate (gain) [1.00]:" );
    ...
    NR_LeakageLabel->setText ( "Leakage (leak) [1.00]:" );
    ...
    ANF_TapsLabel->setText ( "Adaptive Filter Size [45]:" );
    ...
    ANF_DelayLabel->setText ( "Delay [64]:" );
    ...
    ANF_GainLabel->setText ( "Adaptation Rate (gain) [0.01]:" );
    ...
    ANF_LeakageLabel->setText ( "Leakage [0.00001]:" );
    ...
    nbvaluesBox->setGeometry( 5, 5, 330, 50 );
    QLabel *NB_ThresholdLabel = new QLabel ( nbvaluesBox );
    NB_ThresholdLabel->setText ( "Threshold [3.3]:" );
    ...
    QGroupBox *sdromvaluesBox = new QGroupBox ( dspFrame3 );
    sdromvaluesBox->setTitle ( "SDROM Values" );
    sdromvaluesBox->setGeometry( 5, 55, 330, 100 );
    QLabel *SDROM_ThresholdLabel = new QLabel ( sdromvaluesBox );
    SDROM_ThresholdLabel->setText ( "Threshold [2.5]:" );
    SDROM_ThresholdLabel->setGeometry ( 10, 18, 250, 20 );
    ...
    connect ( SDROM_ThresholdSpinBox, SIGNAL ( valueChanged ( double ) ),
              this, SLOT ( setSDROM_Threshold ( double ) ) );

    // Gain Values
    QGroupBox *preampvaluesBox = new QGroupBox ( dspFrame4 );
    preampvaluesBox->setTitle ( "Preamp Gain" );
    preampvaluesBox->setGeometry( 5, 5, 330, 50 );
    QLabel *preampGainLabel = new QLabel ( preampvaluesBox );
    preampGainLabel->setText ( "Gain (dB) [6]:" );
    preampGainLabel->setGeometry ( 10, 18, 250, 20 );
    preampGainLabel->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );
    preampGainSpinBox = new QDoubleSpinBox ( preampvaluesBox );
    preampGainSpinBox->setGeometry (260, 18, 70, 20 );
    preampGainSpinBox->setMinimum( 0 );
    preampGainSpinBox->setMaximum( 20 );
    preampGainSpinBox->setSingleStep( 1 );
    preampGainSpinBox->setValue ( ( double ) preampGain );
    connect ( preampGainSpinBox, SIGNAL ( valueChanged ( double ) ),
              this, SLOT ( setPreamp_Gain ( double ) ) );
    QGroupBox *attvaluesBox = new QGroupBox ( dspFrame4 );
    attvaluesBox->setTitle ( "ATT Gain" );
    attvaluesBox->setGeometry( 5, 55, 330, 100 );
    QLabel *attGainLabel = new QLabel ( attvaluesBox );
    attGainLabel->setText ( "Gain (dB) [-6]:" );
    attGainLabel->setGeometry ( 10, 18, 250, 20 );
    attGainLabel->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );
    attGainSpinBox = new QDoubleSpinBox ( attvaluesBox );
    attGainSpinBox->setGeometry (260, 18, 70, 20 );
    attGainSpinBox->setMinimum( -20 );
    attGainSpinBox->setMaximum( 0 );
    attGainSpinBox->setSingleStep( 1 );
    attGainSpinBox->setValue ( ( double ) attGain );
    connect ( attGainSpinBox, SIGNAL ( valueChanged ( double ) ),
              this, SLOT ( setAtt_Gain ( double ) ) );
    ...
    connect ( lcd, SIGNAL ( tune3 ( int ) ),
              this, SLOT ( tuneff ( int ) ) );
    connect ( lcd, SIGNAL ( tuneStep ( int ) ),
              this, SLOT ( setTuneStep ( int ) ) );

    step_1Hz_frame = new QFrame ( ctlFrame );
    step_1Hz_frame->setGeometry ( 321, 28, 11, 1 );

    step_10Hz_frame = new QFrame ( ctlFrame );
    step_10Hz_frame->setGeometry ( 308, 28, 11, 1 );

    step_100Hz_frame = new QFrame ( ctlFrame );
    step_100Hz_frame->setGeometry ( 295, 28, 11, 1 );

    step_1KHz_frame = new QFrame ( ctlFrame );
    step_1KHz_frame->setGeometry ( 282, 28, 11, 1 );

    step_10KHz_frame = new QFrame ( ctlFrame );
    step_10KHz_frame->setGeometry ( 269, 28, 11, 1 );

    step_100KHz_frame = new QFrame ( ctlFrame );
    step_100KHz_frame->setGeometry ( 256, 28, 11, 1 );

    step_1MHz_frame = new QFrame ( ctlFrame );
    step_1MHz_frame->setGeometry ( 230, 28, 11, 1 );

    step_10MHz_frame = new QFrame ( ctlFrame );
    step_10MHz_frame->setGeometry ( 217, 28, 11, 1 );

    step_100MHz_frame = new QFrame ( ctlFrame );
    step_100MHz_frame->setGeometry ( 204, 28, 11, 1 );
    ...
    QPixmap sdrom_pix ( nb2_xpm );
    SDROM_label = new Varilabel ( swFrame );
    SDROM_label->setLabel( 2 );
    SDROM_label->setPixmap ( sdrom_pix );
    SDROM_label->setGeometry ( 93, 3, 27, 11 );
    connect ( SDROM_label, SIGNAL ( mouseRelease ( int ) ),
              this, SLOT ( toggle_SDROM ( int ) ) );
    connect ( SDROM_label, SIGNAL ( mouseRelease2 ( int ) ),
              this, SLOT ( setDSP ( int ) ) );
    ...
    //------------
    // Preamp / ATT Frame
    QFrame *gainFrame = new QFrame ( this );
    gainFrame->setFrameStyle( QFrame::StyledPanel | QFrame::Plain );
    gainFrame->setPalette( QColor( 100, 0, 0 ) );
    gainFrame->setAutoFillBackground( true );

    QPixmap pre_pix ( pre_xpm );
    preamp_label = new Varilabel ( gainFrame );
    preamp_label->setPixmap ( pre_pix );
    preamp_label->setGeometry ( 3, 3, 27, 11 );
    preamp_label->setAutoFillBackground( true );
    preamp_label->setLabel( 3 );
    connect ( preamp_label, SIGNAL ( mouseRelease ( int ) ),
              this, SLOT ( toggle_preamp ( int ) ) );
    connect ( preamp_label, SIGNAL ( mouseRelease2 ( int ) ),
              this, SLOT ( setDSP ( int ) ) );

    QPixmap att_pix ( att_xpm );
    att_label = new Varilabel ( gainFrame );
    att_label->setPixmap ( att_pix );
    att_label->setGeometry ( 3, 17, 27, 11 );
    att_label->setAutoFillBackground( true );
    att_label->setLabel( 3 );
    connect ( att_label, SIGNAL ( mouseRelease ( int ) ),
              this, SLOT ( toggle_att ( int ) ) );
    connect ( att_label, SIGNAL ( mouseRelease2 ( int ) ),
              this, SLOT ( setDSP ( int ) ) );
    ...
    // Spectrogram auto aperature

    QPixmap manual_pix ( manual_xpm );
    //QPixmap auto_pix ( auto_xpm );
    CAauto_label = new Varilabel ( ctlFrame2 );
    CAauto_label->setFrameStyle( QFrame::StyledPanel | QFrame::Plain );
    CAauto_label->setPalette( QColor( 100, 0, 0 ) );
    CAauto_label->setAutoFillBackground( true );
    CAauto_label->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );
    CAauto_label->setPixmap ( manual_pix );
    CAauto_label->setGeometry (
                M_label->x() + M_label->width() + 1,
                0, 45, 17 );
    connect ( CAauto_label, SIGNAL ( mouseRelease ( int ) ), this, SLOT ( toggle_CAmode ( int ) ) );

    // -----------------------------------------------------------------------
    // Spectrogram Color Aperture

    CA_label = new Varilabel ( ctlFrame2 );
    ...
    CA_label->setGeometry (
                CAauto_label->x() + CAauto_label->width() - 1,
                0,
                font1Metrics->maxWidth() * 14,
                17 );
    CA_label->setAlignment ( Qt::AlignHCenter | Qt::AlignVCenter );
    connect ( CA_label, SIGNAL ( mouseRelease ( int ) ), this, SLOT ( toggleSpectrumAperLines ( int ) ) );
    ...
    AGC_O_label->setGeometry( font1Metrics->maxWidth() * 5, 3, 9, 11 );
    ...
    AGC_L_label->setGeometry( AGC_O_label->x() + 11, 3, 9, 11 );
    ...
    Zoom_in_label->setGeometry( font1Metrics->maxWidth() * 5 + 5, 3, 22, 11 );
    ...
    Zoom_out_label->setGeometry( Zoom_in_label->x() + 22, 3, 22, 11 );
    connect ( Zoom_out_label, SIGNAL ( mouseRelease ( int ) ),
              this, SLOT ( zoomOUT ( int ) ) );

    // -----------------------------------------------------------------------
    QPixmap specmodenorm_pix ( norm_xpm );
    SPECMODE_label = new Varilabel ( ctlFrame2 );
    SPECMODE_label->setFrameStyle( QFrame::StyledPanel | QFrame::Plain );
    SPECMODE_label->setPalette( QColor( 100, 0, 0 ) );
    SPECMODE_label->setPixmap ( specmodenorm_pix );
    SPECMODE_label->setAutoFillBackground( true );
    SPECMODE_label->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );
    SPECMODE_label->setGeometry ( Zoom_label->x() + Zoom_label->width() - 1, 0, 35, 17);
    connect ( SPECMODE_label, SIGNAL ( mouseRelease ( int ) ),
    ...
    WFMODE_label->setPalette( QColor( 100, 0, 0 ) );
    WFMODE_label->setPixmap ( wfmodenorm_pix );
    WFMODE_label->setAutoFillBackground( true );
    WFMODE_label->setAlignment ( Qt::AlignLeft | Qt::AlignVCenter );
    WFMODE_label->setGeometry ( SPECMODE_label->x() + SPECMODE_label->width() - 1, 0, 35, 17);
    ...
    Capture_label->setText ( "Snap" );
    if (capture_auto) {
        Capture_label->setPalette( QColor( 0, 60, 0 ) );
    } else {
        Capture_label->setPalette( QColor( 0, 0, 0 ) );
    }
    Capture_label->setAutoFillBackground( true );
    p = Capture_label->palette();
    p.setColor(QPalette::Active, QPalette::WindowText, QColor( 255, 255, 255) );
    Capture_label->setPalette(p);
    Capture_label->setAlignment ( Qt::AlignHCenter | Qt::AlignVCenter );
    ...
    lcd->setGeometry ( 170, 1, 170, 27 );
    trxFrame->setGeometry ( 337, 4, 19, 11 );
    mhzFrame->setGeometry ( 337, 19, 19, 8 );

    bFrame->setGeometry ( 363, 0, 33, 32 ); // up/dwn
    modeFrame->setGeometry ( 397, 0, 99, 32 );
    swFrame->setGeometry ( 497, 0, 123, 32 ); //NR, ANF, NB, etc.
    gainFrame->setGeometry ( 621, 0, 33, 32 ); // preamp, attenuator
    ...
    cmdFrame->move(geometry.x() + geometry.width()/2 - cmdFrame->width()/2, geometry.y() + geometry.height()/2 - cmdFrame->height()/2);
...
void Main_Widget::loadSettings()
{
    ...
    rx_delta_f = settings->value(
                "/sdr-shell/rx_delta_f", -sample_rate / 4 ).toInt();
    ...
    spectrogramAvgAttack = settings->value(
                "/sdr-shell/spectrogramAvgAttack", 1.0 ).toDouble();
    spectrogramAvgDecay = settings->value(
                "/sdr-shell/spectrogramAvgDecay", 1.0 ).toDouble();
    spectrumAvgAttack = settings->value(
                "/sdr-shell/spectrumAvgAttack", 1.0 ).toDouble();
    spectrumAvgDecay = settings->value(
                "/sdr-shell/spectrumAvgDecay", 1.0 ).toDouble();
    ...
    spectrogramRefresh = settings->value(
...
void Main_Widget::saveSettings()
{
    ...
    // no3m
    if (autoSpecAperture > 0) {
        specApertureLow = specApertureLowTmp;
        specApertureHigh = specApertureHighTmp;
    }
    ...
    settings->setValue ( "/sdr-shell/spectrumAvgDecay", spectrumAvgDecay );
    settings->setValue ( "/sdr-shell/spectrogramAvgGaussBins", spectrogramAvgGaussBins );
    settings->setValue ( "/sdr-shell/spectrumAvgGaussBins", spectrumAvgGaussBins );
...
void Main_Widget::rx_cmd ( int key ) // Leave for IF shift now.
{
    ...
    case '3':
        ...
        break;
    case '4':
        //hScale = sample_rate / (bin_bw * (geometry().width())); // spectrumFrame->width());
        //if (rock_bound) {
        //   rx_delta_f = 0;
        //   setRxFrequency( 0 );
        //}
...
// set relative to carrier freq even in CW modes
#if SPEC_SHIFT
    stop_band = ( (int)(((x + specOffset - (spectrogram->width() / 2)) * bin_bw) * hScale) + rx_delta_f ) / 10*10;
#else
    stop_band = (int)(((x + specOffset - (spectrogram->width() / 2)) * bin_bw) * hScale) / 10*10;
#endif
    if (mode == RIG_MODE_CW || mode == RIG_MODE_USB)
        if (stop_band < 0)
            stop_band = 0;
...
void Main_Widget::readSpectrum()
{
    ...
    for (j = int ((def_spec + int (rx_delta_f/bin_bw))%def_spec), k=0; j < def_spec; ++j, ++k)
       spectrum[k] = raw_spectrum[j];

    for (j = 0; j < int ((def_spec + int (rx_delta_f/bin_bw))%def_spec); ++j, ++k)
       spectrum[k] = raw_spectrum[j];
...
void Main_Widget::drawSpectrogram( int y ) //ok
{
    int x, x1, x2, y1, n;
    int f1, f2;
    int pwr;
    ...
    // did we get a resizevent
    if (windowResize) {
        QPainter p;
        p.begin( this );
        p.translate( QPoint(0, spectrogramTop));
        //p.eraseRect( QRect(0,0,spectrogram->width(),spectrogram->height()));
        p.fillRect(QRect(0,0,spectrogram->width(),spectrogram->height()), QColor( 0, 0, 0 ) ); // faster?
        windowResize = false;
        capture_cntr = 0;
        if (spectrumScrolling == 2) {
          spectrogramPos = 30;
    ...
    // initial avg array with current value
    if (spectrogram_avg_init) {
    ...
    if (spectrogramRefreshCounter >= spectrogramRefresh) {
        spectrogramRefreshCounter = 0;

        x1 = def_spec/2 - spectrum_width/2 + specOffset * hScale;
        x2 = x1 + spectrum_width;

        // avg ffts between refreshes
        ...
        if (spectrogramAvgGaussBins > 0) {
           // Gaussian smoothing across frequency domain
           int spectrogram_smoothing[def_spec];
           int bins = spectrogramAvgGaussBins;
           int len = ( bins * 2 ) + 1;
           float coeff[ len ];
           gaussian_kernel ( coeff, bins );
           for ( x = 0 ; x < def_spec ; ++x ) {
              spectrogram_smoothing[ x ] = 0;
              if ( x - bins > 0 && x + bins < def_spec ) {
                 for ( int j=0 ; j < len ; ++j ) {
                    spectrogram_smoothing[ x ] += spectrogram_data[ x + j - bins ] * coeff[ j ];
                 }
              } else {
                    spectrogram_smoothing[ x ] = spectrogram_data[ x ];
              }
           }

           for ( x = 0 ; x < def_spec ; ++x ) {
              spectrogram_data[x] = spectrogram_smoothing[ x ];
           }
        }

        float ratio;
        for ( x = 0; x < def_spec; ++x ) {
           if (spectrogram_avg[x] < spectrogram_data[x])
              ratio = spectrogramAvgAttack;
           else
              ratio = spectrogramAvgDecay;

           //avg = ( current * a ) + (1 - a) * avg
           spectrogram_avg[x] = ( (spectrogram_data[x] - spectrogram_avg[x]) * ratio ) + spectrogram_avg[x]; // exponential / IIR
        }
        ...
        for ( x = x1; x < x2; x++)		// for number of fft samples per line..
        {
            ...
            pwr = (int)((spectrogram_avg[x] - specApertureLow) * (apertureSize / pwr_range));
        ...
        if ( filterLine ) {
           ...
           // draw filter lines after scaling, sometimes they disappear
        ...
        if (spectrumScrolling == 2) { // linrad
           spectrogramPos--;
           if (spectrogramPos < 0) {
              this->scroll(0,31,QRect(0,spectrogramTop,spectrumFrame->width(),spectrogram->height()));
              p.fillRect( QRect(0,0,spectrumFrame->width(),31), QColor( 0, 0, 0 ) );
              spectrogramPos = 30;
           }
        }
        if (spectrumScrolling == 3) { // linrad reversed
           ...
           if (spectrogramPos > spectrogram->height()-1) {
              this->scroll(0,-31,QRect(0,spectrogramTop,spectrumFrame->width(),spectrogram->height()));
              p.fillRect( QRect(0,spectrogram->height()-31,spectrumFrame->width(),31), QColor( 0, 0, 0 ) );
              spectrogramPos = spectrogram->height()-31;
           }
        }
...
/****************************************************************************************
*       drawFreqScale()  ()     draws the frequency scale on screen                     *
****************************************************************************************/
...
// no3m
    ...
    QPainter p;
    p.begin( this );
    p.translate( QPoint(0, FreqScaleTop));
    p.fillRect( QRect(0, 0, spectrumFrame->width(), freqScale->height() ), QColor( 0, 0, 0 ));
    p.setFont ( *font1 );
    p.setPen( QColor( 255, 255, 255 ) );
    ...
    if ((font1Metrics->maxWidth() * strlen( f_text )) > (2000 / bin_bw / hScale)) {
       ...
       tick = 200;
       label = 2000;
    } else if ((font1Metrics->maxWidth() * strlen( f_text )) > (250 / bin_bw / hScale)) {
       tick = 100;
       label = 1000;
    } else {
       tick = 100;
    ...
    if (freqScaleABSMode) {
       ...
       int band_limits[12][2]
       {
          { 135700, 137800},		{ 472000, 479000},		{ 1800000, 2000000},
          { 3500000, 4000000},		{ 7000000, 7300000},		{ 10100000, 10150000},
    ...
    } else { // relative offset scale per original SDR-shell
       ...
       for ( ; f < sample_rate/2 + abs(rx_delta_f); f += tick) {
#if SPEC_SHIFT
          px = int ( (float (spectrogram->width()) / 2.0 - float (specOffset)) - ( float (rx_delta_f + (f) - cw_offset) / ( bin_bw * hScale ) ) - 1.0 );
#else
...
void Main_Widget::drawPassBandScale()
{
    ...
    p.setPen( Qt::white );
    p.drawLine ( 0, pbScale->height() - 1, pbScale->width(), pbScale->height() - 1 );
    ...
    p.drawLine( f1, 1,
                f1, pbScale->height() - 1);
    sprintf( temp, "%5d", abs(*filter_l) );
    p.drawText( f1 - 12 - font1Metrics->maxWidth() * 5, font1Metrics->ascent(), temp );
    ...
    p.drawLine( f2, 1,
                f2, pbScale->height() - 1); 
    sprintf( temp, "%-5d", abs(*filter_h) );
    p.drawText( f2 + 12, font1Metrics->ascent(), temp );
    ...
    if ( mode == RIG_MODE_CW || mode == RIG_MODE_CWR ) {
       p.drawText(5, font1Metrics->ascent(), "BFO:" + QString::number(CW_tone));
    }
...
void Main_Widget::plotSpectrum( int y )
{
    ...
    static int spectrum_avg[MAX_DEFSPEC];
    static bool spectrum_avg_init = true;
    ...
    //////////////// audio bandpass filter ////// was here, moved below
    ...
    if (spectrum_avg_init) {
       for ( x = 0; x < def_spec; ++x ) {
          spectrum_avg[x] = spectrum_history[y][x];
       }
    ...
    for ( x = 0 ; x < def_spec ; ++x ) {
       if (spectrumAvgGaussBins > 0) {
          if ( x - bins > 0 && x + bins < def_spec ) {
             data = 0;
             for ( int j = 0 ; j < len ; ++j ) {
                data += spectrum_history[ y ][ x + j - bins ] * coeff[ j ];
             }
          } else
             data = spectrum_history[ y ][ x ];

       } else
          data = spectrum_history[ y ][ x ];

       // time-domain IIR filter // no3m
       if (spectrum_avg[x] < data)
          ratio = spectrumAvgAttack;
       else
          ratio = spectrumAvgDecay;

       //avg = ( current * a ) + (1 - a) * avg
       spectrum_avg[x] = ( (data - spectrum_avg[x]) * ratio ) + spectrum_avg[x];

       // store new peaks
       if (spectrumMode == SPEC_PEAK)
          if (spectrum_avg[x] > spectrum_peak[x])
             spectrum_peak[x] = spectrum_avg[x];
    }
    ...
    if (specAvgLine || specPeakMarkers || autoSpecAperture > 0) {

        // only average samples within displayed spectrum
        for (int i=x1; i < x2; ++i){
            spec_avg += spectrum_avg[i];
        }
        spec_avg /= (x2 - x1);
        ...
        for (int i=x1; i < x2; ++i){
            ...
            int spec_tmp = spectrum_avg[i];
            if (spec_tmp < spec_avg + 4) {
                background_avg += spec_tmp;
                ++n;
            }
        }
        if (n != 0){
            ...
            background_avg = spec_avg;
        }
    }

    // auto-aperature
    if (autoSpecAperture > 0) {
        specApertureLow = background_avg + apertureAutoDelta;
        //if (spectrogramAVG) {
           //int adj = log (spectrogramNumAVG) * 5;
           //if (adj > 30) adj = 30;
    ...
    ///////////////////////// fill
    for ( x = 1 ; x < spectrumFrame->width()-1 ; x++ ) {
    ...
    if (specPeakMarkers && spectrumMode != SPEC_PEAK) {
        ...
        while (i < spectrum_width) {
            ...
            if (spectrum_avg[i+x1] > (background_avg + 7)){ // found sig above bg avg
                ...
                // not a good signal
                if (j == spectrumFrame_width_less1 || ((j - i) < 3) || ((j - i) > 75)) { // check sig width
                    i = j + 1;
                    continue;
                } else {
                    // peak position
    ...
    if (specAvgLine && spectrumMode != SPEC_PEAK) {
        ...
        p.setPen( pen );
        p.drawLine( 0, 
                    qBound(0, (int)(spectrumFrame->height() - ((background_avg - specVShift) * vsScale)), spectrumFrame->height()),
                    spectrumFrame->width(), 
                    qBound(0, (int)(spectrumFrame->height() - ((background_avg - specVShift) * vsScale)), spectrumFrame->height()));
    }
...
#if SPEC_SHIFT
    int px = ((spectrogram->width()/2) - specOffset) - ( rx_delta_f / ( bin_bw * hScale ) ) - 1;
    p.drawLine( px,
                0,
                px,
                spectrumFrame->height() -1);
#else
    p.drawLine( (spectrogram->width() / 2) - specOffset - 1,
                0,
                (spectrogram->width() / 2) - specOffset - 1,
                spectrumFrame->height() -1);
#endif
...
#else
      int cw_marker = (spectrogram->width() / 2 + sx1 - specOffset) + (int)(offset / bin_bw / hScale) - 1;
#endif

      p.drawLine( cw_marker,
                  0,
                  cw_marker,
                  spectrumFrame->height() -1);
    }
...
void Main_Widget::spectrogramClicked ( int x )
{
    ...
    if ( !useIF )  // Disable changing frequency for IF mode.  Use arrows for IF shift.
    {
        ...
        //f_old = (rx_f - rx_delta_f) - ( round ( (float)( (rx_f - rx_delta_f) - (int)(bin_bw * (spectrogram->width() / 2 - specOffset) * hScale) ) / (float)step ) * step );
#endif
        ...
        {
            if (mode == RIG_MODE_CW)
               rx_delta_f = f + CW_tone;
            else if (mode == RIG_MODE_CWR)
               rx_delta_f = f - CW_tone;
            else
               rx_delta_f = f + *filter_l + ( *filter_h - *filter_l ) / 2 ;
...
#if SPEC_SHIFT
            if (mode == RIG_MODE_CW)
               rx_f = rx_f + rx_delta_f - f - CW_tone;
            else if (mode == RIG_MODE_CWR)
               rx_f = rx_f + rx_delta_f - f + CW_tone;
            else
               rx_f = rx_f + rx_delta_f - f - *filter_l - ( *filter_h - *filter_l ) / 2;
#else
            if (mode == RIG_MODE_CW)
               rx_f = rx_f - f - CW_tone;
            else if (mode == RIG_MODE_CWR)
...
void Main_Widget::tuneff ( int x )
{
    tunef (x, false);
}

void Main_Widget::tunef ( int x, bool shift = false)
{
    int f_limit = sample_rate/2 - 2000;
    unsigned long long int rx_f_tmp = rx_f;
    int rx_delta_f_tmp = rx_delta_f;
    ...
    if (!rock_bound ) {
        ...
        // invert to match softrock tuning direction
        rx_delta_f -= x;
        //rx_delta_f += x;
        if ( rx_delta_f >  f_limit ) rx_delta_f =  f_limit;
        if ( rx_delta_f < -f_limit ) rx_delta_f = -f_limit;
...
#if SPEC_SHIFT
    // needed if switching to/from CW modes to keep dial shift, but maybe not for regular tuning?? (moving PB vs. moving freq scale)
    if (shift) {
       if (rock_bound) {
          //setSpecOffset(int(int(rx_delta_f - rx_delta_f_tmp)/bin_bw/hScale));
       } else {
...
void Main_Widget::setDSP ( int tab)
{
    dspFrame->show();
    dspTab->setCurrentIndex(tab);
}
...
{
    if(verbose) printf("readMem %d = %lld %lld || spec_width: %d\n", m->getID(), m->getFrequency(), m->getTxFrequency(), spec_width);
    setMode ( ( rmode_t ) m->getMode(), FALSE, FALSE );
    if ( rock_bound ) {
        rx_delta_f = m->getFrequency();
        //if ((rx_delta_f > spec_width) || (rx_delta_f < -spec_width)) {
        if ((rx_delta_f > sample_rate/2) || (rx_delta_f < -(sample_rate/2))) {
...
void Main_Widget::displayMem ( MemoryCell *m )
{
    char temp[32];
    if (rock_bound) {
       snprintf ( temp, 32, "%lf", ( double ) ( rx_f - m->getFrequency() ) / 1000000.0 );
    } else {
...
#if SPEC_SHIFT
    pb_f = ( (int)(((x + specOffset - (spectrogram->width() / 2)) * bin_bw) * hScale) + rx_delta_f ) / 10*10;
    if (mode == RIG_MODE_AM || mode == RIG_MODE_DSB || mode == RIG_MODE_SAM || mode == RIG_MODE_FM)
       pb_f = abs(pb_f);
    else if (mode == RIG_MODE_LSB || mode == RIG_MODE_CWR)
       pb_f = -pb_f;
#else
    pb_f = (int)(((x + specOffset - (spectrogram->width() / 2)) * bin_bw) * hScale) / 10*10;
#endif
...
void Main_Widget::updateCaptureDirectory()
{
    capture_directory = captureDirectoryInput->text();
}
...
void Main_Widget::updateIQGainSlider ( int gain )
{
    iqGain = rxIqGainCourseSlider->value() + rxIqGainFineSlider->value();
    cfgIQGainInput->setValue(iqGain);
    setIQGain();
}
...
void Main_Widget::updateIQPhaseSlider ( int gain )
{
    iqPhase = rxIqPhaseCourseSlider->value() + rxIqPhaseFineSlider->value();
    cfgIQPhaseInput->setValue(iqPhase);
    setIQPhase();
}
...
void Main_Widget::zoomIN ( int )
{
   ...
   int delta = int ( (hScale_old / hScale * float (specOffset)) - float (specOffset) );
   setSpecOffset( delta );
}
...
void Main_Widget::updateFreq(void)
{
    ...
    if (!rock_bound && !transmit){			// only update Freq duing Rx
        if (pUSBCmd->sendCommand("getfreq\n") != 0) return;
...
void Main_Widget::updatePTT(void)
{
    ...
    if (!rock_bound && enableTransmit)
    {
        ...
        if (PTT) {
            if (pUSBCmd->sendCommand("gettone\n") == 0) CW_tone = pUSBCmd->getParam();
...
//== glenoverby commit 9fdc7bf4f693d61cc14d11841a7a75d297e0a670
...
{
    spectrogramNumAVG = value;
}

void Main_Widget::updateSpectrogramAvgAttack ( double value )
{
    spectrogramAvgAttack = value;
}

void Main_Widget::updateSpectrogramAvgDecay ( double value )
{
    spectrogramAvgDecay = value;
}

void Main_Widget::updateSpectrumAvgAttack ( double value )
{
    spectrumAvgAttack = value;
}

void Main_Widget::updateSpectrumAvgDecay ( double value )
{
    spectrumAvgDecay = value;
}
...
void Main_Widget::updateSpectrogramAvgGaussBins ( int value )
{
    spectrogramAvgGaussBins = value;
}

void Main_Widget::updateSpectrumAvgGaussBins ( int value )
{
    spectrumAvgGaussBins = value;
}
...
void Main_Widget::toggleSpectrumAperLines ( int )
{
    if ( specAperLinesButton->isChecked() ) {
        specAperLinesButton->setChecked(false);
        specAperLines = 0;
    } else {
        specAperLinesButton->setChecked(true);
        specAperLines = 1;
    }
}
...
void Main_Widget::toggle_CAmode (int)
{
    QPixmap manual_pix ( manual_xpm );
    QPixmap auto_pix ( auto_xpm );
    QPixmap autoa_pix ( autoa_xpm );
    QPixmap autob_pix ( autob_xpm );
    //autoSpecAperture ^= 1;
    ...
    if (autoSpecAperture == 1) {
        specApertureLowTmp = specApertureLow;
        specApertureHighTmp = specApertureHigh;
        CAauto_label->setPixmap ( autoa_pix );
    } else if (autoSpecAperture == 2) {
        specApertureLowTmp = specApertureLow;
        specApertureHighTmp = specApertureHigh;
        CAauto_label->setPixmap ( autob_pix );
    } else {
        specApertureLow = specApertureLowTmp;
        specApertureHigh = specApertureHighTmp;
        CAauto_label->setPixmap ( manual_pix );
...
{
    if ( specGradientButton->isChecked() )
        spectrumGradient = 1;
    else
        spectrumGradient = 0;
}
...
void Main_Widget::zoom_mouse (int direction)
{
    if (direction > 0)
       zoomIN(direction);
    else
...
{
   int specOffset_tmp = (-rx_delta_f + (((*filter_h - *filter_l) / 2.0 ) + *filter_l )) / bin_bw / hScale;
   setSpecOffset ( specOffset_tmp - specOffset);
}

void Main_Widget::resetSpecOffset ( int )
{
        int cw_offset;
        switch ( mode ) {
           case RIG_MODE_CW:
...
void Main_Widget::gaussian_kernel ( float *coeff, int r )
{
   ...
   //printf("len: %d, sigma: %f, mean: %f, rn: %d\n", len, sigma, mean, rn);

   for (i=0; i < len; ++i) {
      coeff[i] = exp( -(rn * rn) / ( 2.0 * sigma * sigma )) / ( 2.0 * M_PI * sigma * sigma);
      //coeff[i] = exp( -0.5 * pow(rn/sigma, 2.0)) / ( 2 * M_PI * sigma * sigma );
      sum += coeff[i];
      ++rn;
   }
