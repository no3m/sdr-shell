...
void Main_Widget::drawSpectrogram( int y ) //ok
{
    ...
    if (spectrogramRefreshCounter >= spectrogramRefresh) {
        ...
        if ( filterLine ) {
           ...
           if (*filter_l == 0) sx1 = -1;
             else if (*filter_l < 0) sx1 = -2;
           if (*filter_h == 0) sx2 = -1;
             else if (*filter_h < 0) sx2 = -2;
#if SPEC_SHIFT
           f1 = (spectrogram->width() / 2 + sx1 - specOffset) - (int)((rx_delta_f - *filter_l) / bin_bw / hScale);
           //f2 = f1 + (*filter_h - *filter_l) / bin_bw / hScale;
           f2 = (spectrogram->width() / 2 + sx1 - specOffset) - (int)((rx_delta_f - *filter_h) / bin_bw / hScale);
#else
...
void Main_Widget::plotSpectrum( int y )
{
    ...
    if (specAveraging > 1) {
        ...
        for ( x = 1; x < def_spec; x++ ) {
            ...
            spectrum_data[x] = spectrum_data[x] / n;
        }
    }
    ...
    if (specAvgLine || specPeakMarkers || autoSpecAperture > 0) {
#if 0
            // only average samples within visible spectrum
            // average a lot of samples to smooth out jitter
            int k = 0;
            for (y1 = y, n=0; n < 200 && n < spectrogram->height(); n++) {
                for ( x = x1; x < x2; x++ ) {
                    spectrum_avg += spectrum_history[y1][x];
                }
                spectrum_avg /= (x2 - x1);
                if (n == 0) spectrum_avg0 = spectrum_avg;
                for ( x = x1; x < x2; x++ ) {
                   // remove signals
                   if (spectrum_history[y1][x] < spectrum_avg + 4) {
            ...
            if (k > 0){
               ...
               background_avg = spectrum_avg0;
            }
#endif
#if 1
        // old routine, bounces a lot!
        ...
        for (int i=x1; i < x2; i++){
            ...
            if (specAveraging <= 1)
                ...
                spec_tmp = spectrum_data[i];

            if (spec_tmp < spectrum_avg + 4) {
    ...
    if (spectrumMode == SPEC_PEAK) {
        for (int i=0; i<def_spec; i++) {
            if (specAveraging <= 1) {
                ...
                if (spectrum_data[i] > spectrum_peak[i]) {
                    spectrum_peak[i] = spectrum_data[i];
                }
            }
        }
    }
    ...
    for ( x = 0 ; x < spectrum_width + 1 ; ) { 
        ...
        if (hScale > 0.99) { // 1:1 or zoomed out
           ...
           for (int i = 0; i < x_next - x ; ++i) {
              ...
              else if (spectrumMode == SPEC_AVG && spectrum_moving[ x + x1 + i ] > max)
                    max = spectrum_moving[ x + x1 + i ];
              else if (specAveraging <= 1 && spectrum_history[y][ x + x1 + i ] > max)
                    max = spectrum_history[y][ x + x1 + i ];
              else if (spectrum_data[ x + x1 + i ] > max)
        ...
        if (hScale < 0.99) { // zoomed in; interpolate missing bins with simple linear slope
            for (int i = 0; i < sx_next - sx && sx + i < spectrumFrame->width() ; ++i) {
               if (spectrumMode == SPEC_PEAK) {
                  ...
                  spectrum_display[sx+i] = int ( float (spectrum_moving[ x + x1 ]) + ( slope * float (i) ) );
               } else if (specAveraging <= 1) {
                  spectrum_display[sx+i] = int ( float (spectrum_history[y][ x + x1 ]) + ( slope * float (i) ) );
               } else {
                  spectrum_display[sx+i] = int ( float (spectrum_data[ x + x1 ]) + ( slope * float (i) ) );
               }
            }
        }
    ...
    if (specPeakMarkers && spectrumMode != SPEC_PEAK) {
        ...
        while (i < spectrum_width) {
            ...
            //if (spectrum_moving[i+x1] > (background_avg + 6)){ // found sig above bg avg
            if (spectrum_moving[i+x1] > (background_avg + 8)){ // found sig above bg avg
                int j = i + 1;
                while (j < spectrumFrame_width_less1) {
                    //if (spectrum_moving[j+x1] < (background_avg + 6)) break; // found other side of signal
                    if (spectrum_moving[j+x1] < (background_avg + 8)) break; // found other side of signal
                    j++;
                }
                // not a good signal
                if (j == spectrumFrame_width_less1 || ((j - i) < 3) || ((j - i) > 75)) { // check sig width
                    ...
                    if (spectrumMode == SPEC_AVG) {
                        s = spectrum_moving[i+x1];
                        ipk = i;
                        for (int l = i; l <= j; l++) {
                            if (spectrum_moving[l+x1] > s) {
                                ipk = l;
                                s = spectrum_moving[l+x1];
                            }
                        }
                        i = j;
                    } else {
                        if (specAveraging <= 1) {
                            ...
                            s = spectrum_data[i+x1];
                            ipk = i;
                            for (int l = i; l <= j; l++) {
                                if (spectrum_data[l+x1] > s) {
                                    ipk = l;
                                    s = spectrum_data[l+x1];
                                }
                            }
                            i = j;
                        }
    ...
    if (mode == RIG_MODE_CW || mode == RIG_MODE_CWR) {
      ...
      if (offset > 0) sx1 = -2;
      else if (offset <= 0) sx1 = -2;
#if SPEC_SHIFT
      int cw_marker = (spectrogram->width() / 2 + sx1 - specOffset) - (int)((rx_delta_f - offset) / bin_bw / hScale);
#else
      int cw_marker = (spectrogram->width() / 2 + sx1 - specOffset) + (int)(offset / bin_bw / hScale);
#endif
...
//printf("rx_f_tmp: %lu, rx_f: %lu, bin_bw: %f, hScale: %f, specOffset: %d, f: %d, freq: %d\n", rx_f_tmp, rx_f, bin_bw, hScale, specOffset, f, rx_f - f);
        ...
        {
            if (mode == RIG_MODE_CW)
               rx_delta_f = rx_delta_f + f + CW_tone;
            else if (mode == RIG_MODE_CWR)
               rx_delta_f = rx_delta_f + f - CW_tone;
            else
...
printf("rx_f: %d, f: %d, filter_l: %d, filter_h: %d\n", rx_f, f, *filter_l, *filter_h);
            setRxFrequency( 1 );
        }
    }
//printf("rx_f_tmp: %lu, rx_f: %lu, bin_bw: %f, hScale: %f, specOffset: %d, f: %d, freq: %d\n", rx_f_tmp, rx_f, bin_bw, hScale, specOffset, f, rx_f - f);
//printf("rx_f_tmp - rx_f: %d\n", int (rx_f_tmp - rx_f));
...
//rx_f_tmp: 249500, rx_f: 250000, bin_bw: 5.859375, hScale: 1.000000, specOffset: -170
//rx_f_tmp - rx_f: -500
//bin_bw/hScale: -85
//delta: -255
//specOffset: -256
...
#if SPEC_SHIFT
    //pb_f = ( (int)(((x + specOffset - (spectrogram->width() / 2)) * bin_bw) * hScale) + rx_delta_f - cw_offset ) / 10*10;
...
{
   float hScale_old = hScale;
   //if (hScale + 0.2 < float ( (sample_rate - 4000) / (bin_bw * spectrumFrame->width()) ) ) { // cut off 2kHz on edges
...
void Main_Widget::setSpecOffset ( int delta )
{
   ...
   printf ("specOffset_tmp: %d, specOffset: %d, spectrum_width: %d, hScale: %f, def_spec: %d, rx_delta_f: %d, bin_bw: %f\n", 
           specOffset_tmp, specOffset, spectrum_width, hScale, def_spec, rx_delta_f, bin_bw);
}
...
{
   //int specOffset_tmp =  int ( ( ( float (*filter_h - *filter_l) / 2.0 ) + float (*filter_l) ) / bin_bw / hScale);
...
{
   //setSpecOffset( 0 - specOffset );
   //setSpecOffset( -rx_delta_f / bin_bw / hScale );
   //specOffset = 0;
        int cw_offset;
        switch ( mode ) {
           case RIG_MODE_CW:
